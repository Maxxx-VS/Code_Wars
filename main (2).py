...
with open("gamerpg.json", "r", encoding='utf-8') as f:
    data = json.load(f)

@bot.message_handler(commands=["start"])
def start(message: telebot.types.Message):
    bot.send_message(message.chat.id, text="Добро пожаловать в игру The Resident of Mr Charlson!\n"
                                           "Вам предстоит разгадать тайну о пропаже людей в городке Bilsburg.\n"
                                           "А также сделать очень тяжёлый выбор...\n"
                                           "Перед тем, чтобы начать игру необходимо ознокомиться с игрой\n"
                                           "по команде /help.")
...
def help(message: telebot.types.Message):
    bot.send_message(message.chat.id, text="Чтобы начать игру введите команду /letsgo.\n"
                                           ...
                                           "Думайте перед тем, как делать выбор, потому что от этого зависит развитие\n"
                                           "сюжета. Вы можете потерять всё или выйти сухим из воды. В игре есть 4 концовки.\n"
                                           "Удачи :)"
                     )
